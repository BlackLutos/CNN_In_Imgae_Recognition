...
def main():
  ...
  while (key_detect==0):
    ...
    if (times==1):
      ...
      if (labels[label_id]=="0 Clock"):
          print("這是時鐘 不是時中啦~~~","相似度：",prob*100,"%")
      elif (labels[label_id]=="1 Cut"):
          print("哈哈是剪刀啦!!!","相似度：",prob*100,"%")
      else:
          print("甚麼都沒有啦~~~")
    ...
    #cv2.putText(image_src,labels[label_id] + " " + str(round(prob,3)), (5,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 1, cv2.LINE_AA)
    times=times+1
    if (times>1):
      times=1
